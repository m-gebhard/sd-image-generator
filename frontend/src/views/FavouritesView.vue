<template>
    <div class="favourites-view">
        <v-row>
            <v-col>
                <span>{{ favouritesStore.favouritesReversed.length }} Favourites</span>
        ...
        <v-row>
            <v-col
                v-for="(image, i) in favouritesStore.favouritesReversed"
                :key="image.id"
                cols="12"
                md="4"
                xl="2"
...
const onImageClick = (image, i) => {
    lightboxStore.setImagesMeta(favouritesStore.favouritesReversed);
    lightboxStore.open(
        favouritesStore.favouritesReversed.map((img) => img.base64),
        i,
    );
};

const onDownloadAllClick = async () => {
    await zipper(favouritesStore.favouritesReversed.map((img) => img.base64));
};
</script>
