        ...
        <v-row align="center" justify="center">
            <v-col cols="12" md="9">
                <v-select
                    ...
                    hide-details
                    class="mb-0"
                    @update:modelValue="onPresetClick"
                />
            </v-col>
            <v-col cols="12" md="3">
                <v-btn block size="small" color="primary" @click="inputStore.createPreset">
                    New
                </v-btn>
            </v-col>
        </v-row>
...
const onSubmit = async () => {
    ...
    await generatorStore.queueImageJob(
        ...
        inputStore.generationType,
    );
};

const onPresetClick = (presetId) => {
    inputStore.applyPreset(presetId);
    selectedPreset.value = null;
};

const presets = computed(() => [...inputStore.userPresets.reverse()]);
</script>
